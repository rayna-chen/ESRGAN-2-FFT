...
class LQGTDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # modcrop in the validation / test phase
        # if self.opt['phase'] != 'train':
        #     img_WF = util.modcrop(img_WF, scale)
        # print('img_WF_shape')
        # print(img_WF.shape)
        # change color space if necessary
        # if self.opt['color']:
        #     img_WF = util.channel_convert(img_WF.shape[2], self.opt['color'], [img_WF])[0]
        ...
        if self.opt['phase'] == 'train':
            ...
            if H < GT_size or W < GT_size:
                ...
                # if img_WF.ndim == 2:
                #     img_WF = np.expand_dims(img_WF, axis=2)
